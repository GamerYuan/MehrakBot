...
public class GenshinCharacterApplicationServiceTests
{
    ...
    {
        // Arrange
        var (service, characterApiMock, _, _, imageRepositoryMock, imageUpdaterMock, _, gameRoleApiMock, _, _) =
            SetupMocks();
    ...
    public async Task ExecuteAsync_StoresGameUid_WhenNotPreviouslyStored()
    {
        ...
        var context = new GenshinCharacterApplicationContext(1, ("character", "Traveler"), ("server", Server.Asia))
        {
            LtUid = 1ul,
            LToken = "test"
        };
