...
public class GenshinCharListApplicationServiceTests
{
    ...
    {
        // Arrange
        (GenshinCharListApplicationService? service, Mock<ICharacterApiService<GenshinBasicCharacterData, GenshinCharacterDetail, CharacterApiContext>> _, Mock<IImageUpdaterService> _, Mock<IApiService<GameProfileDto, GameRoleApiContext>>? gameRoleApiMock, Mock<ICardService<IEnumerable<GenshinBasicCharacterData>>> _, Mock<IUserRepository> _, Mock<ICharacterCacheService> _) = SetupMocks();
        gameRoleApiMock
    ...
    {
        // Arrange
        (GenshinCharListApplicationService? service, Mock<ICharacterApiService<GenshinBasicCharacterData, GenshinCharacterDetail, CharacterApiContext>>? characterApiMock, Mock<IImageUpdaterService> _, Mock<IApiService<GameProfileDto, GameRoleApiContext>>? gameRoleApiMock, Mock<ICardService<IEnumerable<GenshinBasicCharacterData>>> _, Mock<IUserRepository> _, Mock<ICharacterCacheService> _) = SetupMocks();
    ...
    {
        // Arrange
        (GenshinCharListApplicationService? service, Mock<ICharacterApiService<GenshinBasicCharacterData, GenshinCharacterDetail, CharacterApiContext>>? characterApiMock, Mock<IImageUpdaterService>? imageUpdaterMock, Mock<IApiService<GameProfileDto, GameRoleApiContext>>? gameRoleApiMock, Mock<ICardService<IEnumerable<GenshinBasicCharacterData>>> _, Mock<IUserRepository> _, Mock<ICharacterCacheService> _) = SetupMocks();

        gameRoleApiMock
            .Setup(x => x.GetAsync(It.IsAny<GameRoleApiContext>()))
            .ReturnsAsync(Result<GameProfileDto>.Success(CreateTestProfile()));

        CharacterListData charList = await LoadTestDataAsync<CharacterListData>("CharList_TestData_1.json");
        characterApiMock
            .Setup(x => x.GetAllCharactersAsync(It.IsAny<CharacterApiContext>()))
    ...
    {
        // Arrange
        (GenshinCharListApplicationService? service, Mock<ICharacterApiService<GenshinBasicCharacterData, GenshinCharacterDetail, CharacterApiContext>>? characterApiMock, Mock<IImageUpdaterService>? imageUpdaterMock, Mock<IApiService<GameProfileDto, GameRoleApiContext>>? gameRoleApiMock, Mock<ICardService<IEnumerable<GenshinBasicCharacterData>>>? cardServiceMock, Mock<IUserRepository> _, Mock<ICharacterCacheService>? characterCacheMock) = SetupMocks();
    ...
    {
        // Arrange
        (GenshinCharListApplicationService? service, Mock<ICharacterApiService<GenshinBasicCharacterData, GenshinCharacterDetail, CharacterApiContext>>? characterApiMock, Mock<IImageUpdaterService>? imageUpdaterMock, Mock<IApiService<GameProfileDto, GameRoleApiContext>>? gameRoleApiMock, Mock<ICardService<IEnumerable<GenshinBasicCharacterData>>>? cardServiceMock, Mock<IUserRepository> _, Mock<ICharacterCacheService> _) = SetupMocks();
    ...
    {
        // Arrange
        (GenshinCharListApplicationService? service, Mock<ICharacterApiService<GenshinBasicCharacterData, GenshinCharacterDetail, CharacterApiContext>>? characterApiMock, Mock<IImageUpdaterService> _, Mock<IApiService<GameProfileDto, GameRoleApiContext>>? gameRoleApiMock, Mock<ICardService<IEnumerable<GenshinBasicCharacterData>>> _, Mock<IUserRepository>? userRepositoryMock, Mock<ICharacterCacheService> _) = SetupMocks();
    ...
    {
        // Arrange
        (GenshinCharListApplicationService? service, Mock<ICharacterApiService<GenshinBasicCharacterData, GenshinCharacterDetail, CharacterApiContext>>? characterApiMock, Mock<IImageUpdaterService> _, Mock<IApiService<GameProfileDto, GameRoleApiContext>>? gameRoleApiMock, Mock<IUserRepository> _, Mock<ICharacterCacheService> _) = SetupIntegrationTest();

        gameRoleApiMock
            .Setup(x => x.GetAsync(It.IsAny<GameRoleApiContext>()))
            .ReturnsAsync(Result<GameProfileDto>.Success(CreateTestProfile()));

        CharacterListData charList = await LoadTestDataAsync<CharacterListData>(testDataFile);
        characterApiMock
            .Setup(x => x.GetAllCharactersAsync(It.IsAny<CharacterApiContext>()))
            .ReturnsAsync(Result<IEnumerable<GenshinBasicCharacterData>>.Success(charList.List!));
    ...
    public async Task IntegrationTest_WithRealApi_FullFlow()
    {
        ...
        GenshinCharListApplicationService service = SetupRealApiIntegrationTest();
