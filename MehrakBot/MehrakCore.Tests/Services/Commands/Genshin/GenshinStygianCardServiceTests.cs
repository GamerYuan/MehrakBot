...
public class GenshinStygianCardServiceTests
{
    ...
    // JsonSerializer.DeserializeAsync<StygianData>(
    // File.OpenRead(Path.Combine(AppContext.BaseDirectory, "TestData",
    // "Genshin", "Stygian_TestData_1.json"))); var testData2 = await
    // JsonSerializer.DeserializeAsync<StygianData>(
    // File.OpenRead(Path.Combine(AppContext.BaseDirectory, "TestData",
    // "Genshin", "Stygian_TestData_2.json"))); var testData3 = await
    // JsonSerializer.DeserializeAsync<StygianData>(
    // File.OpenRead(Path.Combine(AppContext.BaseDirectory, "TestData",
    // "Genshin", "Stygian_TestData_3.json")));
    //
    // var image1 = await m_Service.GetStygianCardImageAsync(testData1!,
    // GetTestUserGameData(), Regions.Asia, GetMonsterImages()); var image2 =
    // await m_Service.GetStygianCardImageAsync(testData2!,
    // GetTestUserGameData(), Regions.Asia, GetMonsterImages()); var image3 =
    // await m_Service.GetStygianCardImageAsync(testData3!,
    // GetTestUserGameData(), Regions.Asia, GetMonsterImages());
    //
    ...
    // "TestAssets", "Stygian_GoldenImage_1.jpg")); await using var fileStream2
    // = File.Create(Path.Combine(AppContext.BaseDirectory, "Assets", "Genshin",
    // "TestAssets", "Stygian_GoldenImage_2.jpg")); await using var fileStream3
    // = File.Create(Path.Combine(AppContext.BaseDirectory, "Assets", "Genshin",
