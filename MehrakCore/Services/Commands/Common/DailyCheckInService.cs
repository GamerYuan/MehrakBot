...
public class DailyCheckInService : IDailyCheckInService
{
    ...
    public async Task<Result<(bool, string)>> CheckInAsync(ulong ltuid, string ltoken)
    {
        try
        {
            UserData? userData = await m_GameRecordApiService.GetUserDataAsync(ltuid, ltoken);
            if (userData == null)
                return Result<(bool, string)>.Failure(HttpStatusCode.Unauthorized,
                    "Invalid UID or Cookies. Please re-authenticate your profile");

            Game[] checkInTypes = Enum.GetValues<Game>();

            List<Task<Result<bool>>> tasks = [.. checkInTypes.Select(async type =>
            {
                try
                {
                ...
                catch (Exception ex)
                {
                    ...
                    return Result<bool>.Failure(HttpStatusCode.InternalServerError,
        ...
        catch (Exception e)
        {
            m_Logger.LogError(e, "An error occurred while performing daily check-in for user LtUid: {UserId}",
                ltuid);
            return Result<(bool, string)>.Failure(HttpStatusCode.InternalServerError,
                "An unknown error occurred while performing daily check-in");
        }
    }

    private async Task<Result<bool>> CheckInHelperAsync(Game type, ulong ltuid, string ltoken)
    {
        if (!CheckInUrls.TryGetValue(type, out string? url) || !CheckInActIds.TryGetValue(type, out string? actId))
        {
            ...
            return Result<bool>.Failure(HttpStatusCode.BadRequest, "Invalid check-in type");
        }
        ...
        if (!response.IsSuccessStatusCode)
        {
            ...
            return Result<bool>.Failure(response.StatusCode, "An unknown error occurred during check-in");
        }
