...
public class ZzzAssaultCommandExecutorTests
{
    private ZzzAssaultCommandExecutor m_Executor;
    private Mock<IHttpClientFactory> m_HttpClientFactoryMock;
    private ImageRepository m_ImageRepository;
    private UserRepository m_UserRepository;
    private ZzzAssaultCardService m_CardService;
    private ZzzAssaultApiService m_ApiService;
    ...
    public async Task Setup()
    {
        ...
        m_Executor = new ZzzAssaultCommandExecutor(m_ApiService, m_CardService,
            ...
            gameRecordApiService, Mock.Of<ILogger<ZzzAssaultCommandExecutor>>())
        {
    ...
    public async Task ExecuteAsync_WithAssaultApiNoData_ShouldSendNoRecordsMessage()
    {
        ...
        await m_Executor.ExecuteAsync(Regions.Asia, TestProfileId);
        string? message = await m_DiscordTestHelper.ExtractInteractionResponseDataAsync();
        Assert.That(message, Does.Contain("No Deadly Assault clear records found"));
    }
        ...
        m_HttpMessageHandlerMock.Protected()
            .Setup<Task<HttpResponseMessage>>(
                ...
                ItExpr.Is<HttpRequestMessage>(req => req.RequestUri != null &&
                                                     req.RequestUri!.GetLeftPart(UriPartial.Path) == AssaultApiUrl),
    ...
    public async Task ExecuteAsync_AssaultApiErrorRetcode_ShouldSendGenericAssaultErrorMessage()
    {
        ...
        await m_Executor.ExecuteAsync(Regions.Asia, TestProfileId);
        string? message = await m_DiscordTestHelper.ExtractInteractionResponseDataAsync();
        Assert.That(message, Does.Contain("An error occurred while fetching Deadly Assault data"));
    }
    ...
    {
        string testDataPath = Path.Combine(AppContext.BaseDirectory, "TestData", "Zzz", "Da_TestData_1.json");
    ...
    private async Task SetupApiServiceDictionary()
    {
        Type type = m_ApiService.GetType();
        if (type.GetField("m_BossImage", BindingFlags.NonPublic | BindingFlags.Instance)!.GetValue(m_ApiService)
                is not ConcurrentDictionary<string, Stream> bossImage ||
            type.GetField("m_BuffImage", BindingFlags.NonPublic | BindingFlags.Instance)!.GetValue(m_ApiService)
                is not ConcurrentDictionary<string, Stream> buffImage)
            throw new InvalidOperationException("Failed to access private fields of ZzzAssaultApiService.");

        bossImage.Clear();
        buffImage.Clear();

        Stream deadEndButcher = await m_ImageRepository.DownloadFileToStreamAsync("Notorious_DeadEndButcher");
        bossImage.TryAdd("Notorious - Dead End Butcher", deadEndButcher);
        bossImage.TryAdd("Miasma Priest",
            await m_ImageRepository.DownloadFileToStreamAsync("MiasmaPriest"));
        bossImage.TryAdd("Autonomous Assault Unit - Typhon Destroyer",
            await m_ImageRepository.DownloadFileToStreamAsync("AutonomousAssaultUnit_TyphonDestroyer"));

        buffImage.TryAdd("Lingering Illness",
            await m_ImageRepository.DownloadFileToStreamAsync("Buff_LingeringIllness"));
        buffImage.TryAdd("Sonata",
            await m_ImageRepository.DownloadFileToStreamAsync("Buff_Sonata"));
        buffImage.TryAdd("Power Boost",
