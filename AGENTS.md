...
- **.NET 10 SDK**
- **Aspire CLI** — install globally: `dotnet tool install -g aspire`
...
- **Git LFS + submodules** — assets live in `MehrakBot/Assets/` as a submodule with LFS files:
  ...
  git lfs pull
  ```
- **Local configuration** — create `.env.local` from `.env.template` for Compose, place the ImageSharp license at `MehrakBot/sixlabors.lic`, and ensure the ignored `seaweed-s3/s3.json` exists before running Aspire.
...
MehrakBot/
  ...
  Services/
    ...
    MigrationService/      — hosted EF Core migration runner
  ...
  # Test projects cover Domain, GameApi, Infrastructure, Application, Bot,
  # Dashboard, and ImageProcessor.
```
...
**Seven EF DbContexts** in `Mehrak.Infrastructure`: `CharacterDbContext`, `RelicDbContext`, `UserDbContext`, `CodeRedeemDbContext`, `DashboardAuthDbContext`, `DocumentationDbContext`, `ReleaseNoteDbContext`. Migration commands must specify `--context <Name>`. Do not hand-edit generated migration files; `MigrationService` applies all seven at AppHost startup.
...
- **Release image builds** (`build.yml`): triggered by `v*` tags; builds and pushes Application, Bot, Dashboard, and ImageProcessor images to GHCR
- **GitHub releases** (`release.yml`): triggered by `v*` tags; archives `MehrakBot/Assets` and creates the release
...
Use git conventional commits when writing commit messages. Keep them short and concise
